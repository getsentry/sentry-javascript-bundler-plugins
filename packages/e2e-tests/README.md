...
When `yarn test` is run, first `setup.ts` in all scenarios is executed, afterwards we run `vitest run`, which will pick up all `*.test.ts` files.
...
- `*.test.ts` specifies all tests which are executed by Vitest for this scenario.
